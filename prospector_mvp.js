...
import { SYSTEM_PROMPT, getScoringUserPrompt } from './scoring_prompt.js';
...
if (!apiKey || apiKey === 'tu_api_key_aqui') {
  console.error('❌ ERROR: Debes configurar la variable OPENAI_API_KEY en tu archivo .env');
  process.exit(1);
}
 ...
 */
async function scrollFeed(page, maxResults = 20) {
  ...
  while (true) {
    ...
    if (uniqueLinks.length === previousCount) {
      ...
      if (noChangeCount >= 5) {
        console.log('   🏁 Se llegó al final de los resultados disponibles.');
        return uniqueLinks;
      }
    } else {
      noChangeCount = 0;
...
async function extractBusinessDetails(page, url) {
  ...
  try {
    ...
    await page.waitForTimeout(1000); // Pequeña pausa para asegurar la carga asíncrona de datos secundarios
...
/**
 * Califica el negocio utilizando la API de OpenAI
 */
async function scoreBusinessWithOpenAI(business) {
  console.log(`🧠 Calificando a "${business.nombre}" con OpenAI...`);
  ...
  try {
    ...
    console.log(`   🔥 Calificación: ${parsedResult.rating} (Score: ${parsedResult.score}/100)`);
    console.log(`   💡 Oportunidades: ${Array.isArray(parsedResult.oportunidades) ? parsedResult.oportunidades.join(', ') : parsedResult.oportunidades}`);
    console.log(`   💰 Presupuesto estimado: ${parsedResult.presupuesto_estimado}`);

    return {
      score: parsedResult.score ?? 'sin datos',
      rating_openai: parsedResult.rating ?? 'sin datos',
      reasoning: parsedResult.reasoning ?? 'sin datos',
      oportunidades: Array.isArray(parsedResult.oportunidades) ? parsedResult.oportunidades.join('; ') : (parsedResult.oportunidades ?? 'sin datos'),
      presupuesto_estimado: parsedResult.presupuesto_estimado ?? 'sin datos',
      priority_reason: parsedResult.priority_reason ?? 'sin datos'
    };
  } catch (error) {
    console.error(`⚠️ Error al calificar con OpenAI para "${business.nombre}":`, error.message);
    return {
      score: 'sin datos',
      rating_openai: 'sin datos',
      reasoning: 'Error en calificación de OpenAI',
      oportunidades: 'sin datos',
      presupuesto_estimado: 'sin datos',
      priority_reason: 'sin datos'
    };
  }
}
...
function escapeCsvValue(val) {
  ...
  // Si contiene comillas, comas o saltos de línea, lo escapamos según el estándar RFC 4180
...
/**
 * Guarda los leads finales calificados en un archivo CSV compatible con Excel en español
 */
function saveLeadsToCsv(leads, filename = 'leads.csv') {
  const headers = [
    'nombre', 'categoria', 'rating', 'reseñas', 'telefono', 'website', 
    'direccion', 'mapsUrl', 'score', 'rating_openai', 'reasoning', 
    'oportunidades', 'presupuesto_estimado', 'priority_reason'
  ];
  ...
  for (const lead of leads) {
    const row = [
      ...
      escapeCsvValue(lead.rating_openai),
      escapeCsvValue(lead.reasoning),
      escapeCsvValue(lead.oportunidades),
      escapeCsvValue(lead.presupuesto_estimado),
      escapeCsvValue(lead.priority_reason)
    ];
    rows.push(row.join(','));
  }

  // Escribir archivo agregando UTF-8 BOM (\ufeff) para compatibilidad directa con Excel en Español
 ...
 */
async function main() {
  const query = 'veterinarias en Satélite Naucalpan';
  const maxLeads = 20;
  const isHeadless = process.env.HEADLESS !== 'false'; // por defecto headless
  
  console.log('🚀 === PROSPECTOR-ON MVP INICIADO ===');
  ...
  const browser = await chromium.launch({
    headless: isHeadless,
    args: ['--disable-blink-features=AutomationControlled'] // Ocultar automatización básica
  });
  ...
  try {
    // Ir directamente al buscador de Google Maps con el query codificado
    ...
    // Procesar cada negocio de uno en uno de forma robusta
    for (let i = 0; i < leadUrls.length; i++) {
      ...
      // Solo calificar si al menos obtuvimos el nombre del negocio
      if (rawDetails.nombre && rawDetails.nombre !== 'sin datos') {
        const scoreDetails = await scoreBusinessWithOpenAI(rawDetails);
        finalLeads.push({
          ...rawDetails,
          ...scoreDetails
        });
      } else {
        console.log('⚠️ Omitiendo calificación por falta de datos básicos.');
        finalLeads.push({
          ...rawDetails,
          score: 'sin datos',
          rating_openai: 'sin datos',
          reasoning: 'Sin datos de negocio suficientes',
          oportunidades: 'sin datos',
          presupuesto_estimado: 'sin datos',
          priority_reason: 'sin datos'
        });
      ...
      // Delay preventivo para no saturar servidores ni ser detectados
      await page.waitForTimeout(1000);
    }
    
    // Guardar los resultados calificados en el CSV
    saveLeadsToCsv(finalLeads, 'leads.csv');
